...
import type { ReadinessStatus, Release } from '../types/release'

export const useReleaseStore = defineStore('releases', () => {
  const releases = ref<Release[]>(structuredClone(seedReleases))
  ...
  function setCheckStatus(releaseId: string, checkId: string, status: ReadinessStatus) {
